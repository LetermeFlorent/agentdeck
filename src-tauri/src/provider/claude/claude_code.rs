...
pub(crate) fn emit(app: &tauri::AppHandle, id: &str, ev: SessionEvent) {
...
pub(crate) fn write_images(images: &[ImageInput]) -> Vec<String> {
    if images.is_empty() {
        return Vec::new();
    }
...
pub async fn send(
    ...
    let mut ok = false;
    if let Some(stdin) = guard.as_mut().and_then(|p| p.stdin.as_mut()) {
        ok = write_line(stdin, &msg).await.is_ok();
    }
    if !ok {
        if let Some(old) = guard.take() {
            let mut c = old.child;
